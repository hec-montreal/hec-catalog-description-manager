...
public class CatalogDescriptionDaoImpl extends HibernateDaoSupport implements
	...
	public CatalogDescription getCatalogDescription(String course_id) {
		// there should only ever be one active description, but it can't hurt to order by db id.
		DetachedCriteria dc = DetachedCriteria.forClass(CatalogDescription.class)
				.add(Restrictions.eq("courseId", course_id.toUpperCase()))
				.add(Restrictions.eq("active", true))
				.addOrder(Order.desc("id"));
		
		return (CatalogDescription)getHibernateTemplate().findByCriteria(dc).get(0);
	}
